...
import { getOpenSellerCollectionsBySeller, getOpenSellerPayablesBySeller, getStockSummaryRows, } from "@/lib/stock-runtime-data";
...
export async function getDashboardSnapshot(role) {
    ...
    const [todaySales, todaySaleItems, stockSummary, lowStock, receivableAggregate, supplierAggregate, sellerPayables, sellerCollections, recentSales, recentPurchases, recentExpenses, recentSettlements, recentCollections, sevenDaySales,] = await Promise.all([
    ...
    const todayCashSales = sumRows(todaySales
        .filter((sale) => sale.paymentMethod === "CASH")
        .map((sale) => toNumber(sale.total)));
    const todayProfit = Number(todaySaleItems
        .reduce((sum, saleItem) => {
        ...
        .toFixed(2));
    const todayBankSales = sumRows(todaySales
        .filter((sale) => sale.paymentMethod === "BANK")
        .map((sale) => toNumber(sale.total)));
    const todayCreditSales = sumRows(todaySales
