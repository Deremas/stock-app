...
export async function getPurchaseRows(filters = {}) {
...
export async function getSupplierRows(branchId) {
    ...
    return suppliers.map((supplier) => {
        const payableBalance = sumRows(supplier.purchases.map((purchase) => toNumber(purchase.amountDue)));
        return {
            id: supplier.id,
            name: supplier.name,
...
export async function getSupplierPaymentRows(filters = {}) {
...
export async function getFinanceAccountRows(filters = {}) {
...
export async function getCashAccountRows(branchId) {
    return getFinanceAccountRows({
        ...(branchId ? { branchId } : {}),
        accountType: "CASH",
    });
}
export async function getCashTransferRows(branchId) {
    ...
    return [...transferMap.values()].sort((left, right) => right.transferDate.localeCompare(left.transferDate));
}
export async function getExpenseRows(branchId) {
...
export async function getExpenseCategorySummaryRows(filters = {}) {
...
export async function getExpenseKpis(branchId) {
    const rows = await prisma.expense.findMany({
        where: {
            ...(branchId ? { branchId } : {}),
        },
...
export async function getLedgerRows(branchId) {
    const rows = await prisma.ledgerEntry.findMany({
        where: {
            ...(branchId
                ? {
...
export async function getUserRows() {
    ...
    return rows.map((row) => ({
        ...
        branches: row.branchAssignments.map((assignment) => assignment.branch.name).join(", ") ||
            "-",
        status: row.isActive ? "ACTIVE" : "INACTIVE",
    }));
}
...
export async function getAuditLogRows(branchId) {
