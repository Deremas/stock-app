...
function itemAllowed(itemRoles, permission, role) {
...
export function getNavigationTitle(pathname) {
    const items = navigationEntries.flatMap((entry) => entry.type === "link" ? [entry] : entry.items);
    const candidates = [...items, ...hiddenPageTitles]
        .filter((item) => pathname === item.href || pathname.startsWith(`${item.href}/`))
        .sort((left, right) => right.href.length - left.href.length);
    return candidates[0]?.title ?? "Stock Management";
}
export function getOpenGroupForPath(pathname, role) {
    ...
    const hiddenMatch = hiddenGroupPrefixes.find((item) => pathname === item.href || pathname.startsWith(`${item.href}/`));
