...
function createBatchListHref(input) {
    const params = new URLSearchParams({
        productId: input.productId,
...
function createEditItemHref(productId) {
    const params = new URLSearchParams({
        productId,
        mode: "edit",
        open: "1",
...
export async function getProductRows(branchId) {
...
export async function getStockOverviewRows(branchId) {
    ...
    for (const batch of ownedBatches) {
        ...
        const batchLine = `${batch.sourceType === "PURCHASE" ? batch.referenceNumber : `${batch.referenceNumber} from ${batch.sourceName}`} | ${batch.remainingQuantity} qty | Buy ${formatCurrency(batch.unitCost)} | Sell ${formatCurrency(batch.sellingPrice)}`;
        if (!existing) {
...
export async function getLowStockRows(branchId) {
    const stockSummary = await getStockSummaryRows(branchId);
    return stockSummary
        .filter((row) => row.minimumStockAlert > 0 && row.totalQty <= row.minimumStockAlert)
        .map((row) => ({
        id: row.id,
        branch: row.branch,
        name: row.product,
        ...
        status: row.totalQty <= Math.max(1, Math.floor(row.minimumStockAlert / 2))
            ? "CRITICAL"
            : "LOW",
    }));
}
export async function getOutOfStockRows(branchId) {
    const stockSummary = await getStockSummaryRows(branchId);
    return stockSummary
...
export async function getAlertRecordRows(branchId) {
...
export async function getStockMovementRows(branchId) {
...
export async function getTransferRows(branchId) {
