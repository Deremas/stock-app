...
export async function getOpenSellerPayablesBySeller(branchId) {
    try {
        ...
        for (const allocation of allocations) {
            const sellerId = (allocation.sellerAssignmentItem?.sellerIntakeItemId
            ...
            const settled = sumRows(allocation.settlementAllocations.map((item) => toNumber(item.amount)));
...
export async function getOpenSellerCollectionsBySeller(branchId) {
    try {
        const allocations = await prisma.saleItemAllocation.findMany({
            where: {
        ...
        for (const allocation of allocations) {
            ...
            const gross = toNumber(allocation.sellerAmount) * allocation.quantity;
            const collected = sumRows(allocation.collectionAllocations.map((item) => toNumber(item.amount)));
            collections.set(sellerId, (collections.get(sellerId) ?? 0) + gross - collected);
        }
        return collections;
    }
    catch (error) {
...
export async function getStockSummaryRows(branchId) {
    noStore();
    try {
