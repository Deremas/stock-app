...
function getSoldAtRangeFilter(filters) {
...
export async function getSalesRows(filters = {}) {
...
export async function getSoldItemRows(filters = {}) {
    const soldAt = getSoldAtRangeFilter(filters);
    const saleFilter = filters.branchId || filters.customerId || soldAt
        ? {
            ...(filters.branchId ? { branchId: filters.branchId } : {}),
            ...(filters.customerId ? { customerId: filters.customerId } : {}),
            ...(soldAt ? { soldAt } : {}),
    ...
    return saleItems.map((saleItem) => {
        ...
        const sellers = [
            ...new Set(saleItem.allocations
                .map((item) => item.sellerAssignmentItem?.sellerAssignment.seller.fullName ??
...
export async function getSalesProfitRows(filters = {}) {
    const soldAt = getSoldAtRangeFilter(filters);
    const saleItems = await prisma.saleItem.findMany({
        where: {
            sale: {
    ...
    return saleItems.map((saleItem) => {
        ...
        const isPartnerOwnedAllocation = (allocation) => allocation.sourceType === "SELLER_CONSIGNMENT" ||
            (allocation.sourceType === "SELLER_ASSIGNED" &&
                Boolean(allocation.sellerAssignmentItem?.sellerIntakeItemId));
        const partnerPayable = Number(saleItem.allocations
            ...
            .toFixed(2));
        const costTotal = Number(saleItem.allocations
            .reduce((sum, allocation) => {
            if (!isPartnerOwnedAllocation(allocation)) {
...
export async function getCustomerRows(filters = {}) {
...
export async function getCustomerCreditRows(filters = {}) {
...
export async function getCustomerPaymentRows(filters = {}) {
