...
import { getSellerCollectionRows, getSellerIntakeRows, getSellerReturnRows, getSellerSettlementRows, } from "@/lib/page-data-sellers";
...
export async function getDailyCheckSnapshot(args) {
    ...
    const [salesRows, soldItemRows, partnerIntakeRows, partnerPayoutRows, partnerCollectionRows, partnerReturnRows, partnerSaleAllocations, expenseAggregate,] = await Promise.all([
        getSalesRows({
    ...
    const postedPartnerPayoutRows = partnerPayoutRows.filter((row) => row.status === "POSTED");
    const postedPartnerCollectionRows = partnerCollectionRows.filter((row) => row.status === "POSTED");
    const totalSales = sumRows(salesRows.map((row) => toNumber(row.total)));
    const cashSales = sumRows(salesRows
        .filter((row) => row.paymentMethod === "CASH")
        .map((row) => toNumber(row.total)));
    const bankSales = sumRows(salesRows
        .filter((row) => row.paymentMethod === "BANK")
        .map((row) => toNumber(row.total)));
    const creditSales = sumRows(salesRows
        .filter((row) => row.paymentMethod === "CREDIT")
        .map((row) => toNumber(row.total)));
    const partnerItemsReceived = sumRows(partnerIntakeRows.map((row) => toNumber(row.quantityBrought)));
    const partnerReturnQuantity = sumRows(partnerReturnRows.map((row) => toNumber(row.quantity)));
    const partnerReceivedSoldAmount = Number(partnerSaleAllocations
        .reduce((sum, allocation) => {
        const isPartnerOwned = allocation.sourceType === "SELLER_CONSIGNMENT" ||
        ...
        .toFixed(2));
    const partnerPayableAmount = Number(partnerSaleAllocations
        .reduce((sum, allocation) => {
        const isPartnerOwned = allocation.sourceType === "SELLER_CONSIGNMENT" ||
            (allocation.sourceType === "SELLER_ASSIGNED" &&
                Boolean(allocation.sellerAssignmentItem?.sellerIntakeItemId));
        ...
        .toFixed(2));
    const partnerAssignedSoldAmount = Number(partnerSaleAllocations
        .reduce((sum, allocation) => {
        const isAssignedFromUs = allocation.sourceType === "SELLER_ASSIGNED" &&
        ...
        .toFixed(2));
    const partnerCollectionAmount = Number(partnerSaleAllocations
        .reduce((sum, allocation) => {
        const isAssignedFromUs = allocation.sourceType === "SELLER_ASSIGNED" &&
            !allocation.sellerAssignmentItem?.sellerIntakeItemId;
        if (!isAssignedFromUs) {
        ...
        .toFixed(2));
    const partnerPaidOutAmount = sumRows(postedPartnerPayoutRows.map((row) => toNumber(row.amount)));
    const partnerCollectedAmount = sumRows(postedPartnerCollectionRows.map((row) => toNumber(row.amount)));
    const expenseTotal = toNumber(expenseAggregate._sum.amount);
    const returnedToPartnerQty = sumRows(partnerReturnRows
        .filter((row) => row.flow === "BACK_TO_PARTNER")
        .map((row) => toNumber(row.quantity)));
    const returnedToBranchQty = sumRows(partnerReturnRows
