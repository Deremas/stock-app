...
import { buildGeneratedUsername, getUserDisplayUsername, getUserLoginLabel, } from "@/lib/user-login";
import { buildArchivedUsername, isArchivedUsername } from "@/lib/user-archive";
import { userDeleteSchema, userSchema, userStatusSchema, userUpdateSchema, } from "@/lib/validation/user";
...
async function validateAssignedBranches(tx, branchIds) {
...
async function assertUniqueUserIdentity(args) {
    ...
    const duplicatePhone = args.normalizedPhone
        ? existingPhoneUsers.find((user) => normalizePhoneIdentifier(user.phone ?? "") === args.normalizedPhone)
        : null;
    if (duplicatePhone) {
        throw new Error("That phone number is already linked to another user.");
    }
}
function buildBranchAssignments(branchIds, defaultBranchId) {
...
async function getManagedUser(tx, userId) {
...
async function ensureUserCanBeDisabledOrDeleted(args) {
    if (args.user.id === args.actorId) {
        throw new Error(args.action === "delete"
...
export async function setUserActiveStateAction(input) {
    const actor = await getActionActor(["ADMIN"]);
    if (!actor) {
        return {
            success: false,
    ...
    try {
        ...
        return {
            success: false,
            message: getActionErrorMessage(error, "Unable to change the user status right now."),
        };
    }
}
export async function deleteUserAction(input) {
    ...
    if (!parsed.success) {
        return {
            success: false,
            message: parsed.error.issues[0]?.message ?? "User delete request is invalid.",
        };
    }
    try {
