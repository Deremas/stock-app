...
import { LedgerDirection, LedgerEntryType, PaymentStatus, PurchaseStatus, StockMovementType, StockOwnershipType, } from "@/generated/prisma/enums";
...
export async function createPurchaseAction(input) {
    ...
    if (!parsed.success) {
        return {
            success: false,
            message: parsed.error.issues[0]?.message ?? "Purchase payload did not validate.",
        };
    }
    ...
    try {
        const purchaseReference = await prisma.$transaction(async (tx) => {
            ...
            if (!supplier && parsed.data.settlementMode !== "FULL") {
                throw new Error("Choose a supplier for unpaid or partial purchases so the balance can be settled later.");
            }
            ...
            if (parsed.data.settlementMode !== "UNPAID" &&
                paymentAccount &&
            ...
            const amountPaid = parsed.data.settlementMode === "UNPAID"
                ? 0
        ...
        return {
            success: false,
            message: getActionErrorMessage(error, "Unable to post the purchase right now."),
        };
    }
}
