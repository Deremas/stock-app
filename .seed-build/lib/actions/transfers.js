...
export async function createTransferAction(input) {
    ...
    if (!parsed.success) {
        return {
            success: false,
            message: parsed.error.issues[0]?.message ?? "Transfer payload did not validate.",
        };
    }
    ...
    try {
        const transferReference = await prisma.$transaction(async (tx) => {
            ...
            const sourceBranch = assignments.find((assignment) => assignment.branchId === parsed.data.sourceBranchId)?.branch;
            const destinationBranch = assignments.find((assignment) => assignment.branchId === parsed.data.destinationBranchId)?.branch;
            if (!sourceBranch || !destinationBranch) {
                throw new Error("You must be assigned to both the source and destination branches.");
            }
            ...
            for (const item of parsed.data.items) {
                ...
                if (remainingQuantity < item.quantity) {
                    throw new Error(`${sourceBatch.productName} does not have enough remaining quantity in the selected batch.`);
                }
        ...
        return {
            success: false,
            message: getActionErrorMessage(error, "Unable to post the transfer right now."),
        };
    }
}
