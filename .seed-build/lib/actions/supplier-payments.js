...
export async function createSupplierPaymentAction(input) {
    ...
    if (!parsed.success) {
        return {
            success: false,
            message: parsed.error.issues[0]?.message ?? "Supplier payment details are invalid.",
        };
    }
    ...
    try {
        const paymentReference = await prisma.$transaction(async (tx) => {
            ...
            if (financeAccount.branchId && financeAccount.branchId !== purchase.branchId) {
                throw new Error("Payment account must belong to the same branch as the outstanding purchase.");
            }
        ...
        return {
            success: false,
            message: getActionErrorMessage(error, "Unable to record the supplier payment right now."),
        };
    }
}
