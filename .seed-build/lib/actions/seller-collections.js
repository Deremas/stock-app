...
export async function createSellerCollectionAction(input) {
    ...
    if (!parsed.success) {
        return {
            success: false,
            message: parsed.error.issues[0]?.message ?? "Partner collection details are invalid.",
        };
    }
    ...
    try {
        const collectionReference = await prisma.$transaction(async (tx) => {
            ...
            if (financeAccount.branchId && financeAccount.branchId !== branch.id) {
                throw new Error("Receiving account must belong to the same branch as the collection.");
            }
            ...
            const allocationMap = new Map(allocations.map((allocation) => {
                const amountDue = Number((Number(allocation.sellerAmount ?? 0) * allocation.quantity -
                    allocation.collectionAllocations.reduce((sum, item) => sum + Number(item.amount), 0)).toFixed(2));
                return [
                    allocation.id,
                    {
                        id: allocation.id,
            ...
            const normalizedItems = parsed.data.items.map((item) => {
                ...
                if (allocation.amountDue <= 0) {
                    throw new Error(`${allocation.productName} on ${allocation.saleNumber} is already fully collected.`);
                }
                const normalizedAmount = Number(Number(item.amount).toFixed(2));
                if (normalizedAmount > allocation.amountDue) {
                    throw new Error(`${allocation.productName} on ${allocation.saleNumber} has only ETB ${allocation.amountDue.toFixed(2)} left to collect.`);
                }
                return {
                    ...item,
            ...
            const totalAmount = Number(normalizedItems.reduce((sum, item) => sum + item.amount, 0).toFixed(2));
            if (totalAmount <= 0) {
        ...
        return {
            success: false,
            message: getActionErrorMessage(error, "Unable to record the partner collection right now."),
        };
    }
}
