...
function parseReturnLineId(lineId) {
...
export async function createSellerReturnAction(input) {
    ...
    if (!parsed.success) {
        return {
            success: false,
            message: parsed.error.issues[0]?.message ?? "Return details are invalid.",
        };
    }
    ...
    try {
        const returnReference = await prisma.$transaction(async (tx) => {
            ...
            for (const item of parsed.data.items) {
                ...
                if (parsedLine.kind === "INTAKE") {
                    ...
                    const availableQty = intakeItem.quantityBrought -
                        intakeItem.quantityAssigned -
                        intakeItem.quantitySold -
                        intakeItem.quantityReturned;
                    if (availableQty < item.quantity) {
                        throw new Error(`${intakeItem.product.name} has only ${availableQty} item(s) left to return to the partner.`);
                    }
                ...
                const availableQty = assignmentItem.quantityAssigned -
                    assignmentItem.quantitySold -
                    assignmentItem.quantityReturned;
                if (availableQty < item.quantity) {
                    throw new Error(`${assignmentItem.product.name} has only ${availableQty} item(s) left to return back into stock.`);
                }
                const returnItem = await tx.sellerReturnItem.create({
                    data: {
                        sellerReturnId: sellerReturn.id,
            ...
            await createAuditLog(tx, {
                ...
                after: {
                    ...
                    totalQuantity: parsed.data.items.reduce((sum, current) => sum + current.quantity, 0),
                },
            });
        ...
        return {
            success: false,
            message: getActionErrorMessage(error, "Unable to record the partner return right now."),
        };
    }
}
