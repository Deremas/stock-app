...
import { branchSchema, branchUpdateSchema, } from "@/lib/validation/branch";
...
function buildBranchCodeSeed(name) {
    const seed = name.toUpperCase().replace(/[^A-Z0-9]/g, "");
    return seed || "BRANCH";
}
async function generateUniqueBranchCode(tx, name, ignoreBranchId) {
    const base = buildBranchCodeSeed(name);
    let candidate = base;
    let suffix = 1;
    while (await tx.branch.findFirst({
        where: {
            code: candidate,
...
export async function deleteBranchAction(input) {
    ...
    try {
        const deletedBranchName = await prisma.$transaction(async (tx) => {
            ...
            if (hasActivity) {
                throw new Error("This branch already has stock or transaction history and cannot be deleted.");
            }
