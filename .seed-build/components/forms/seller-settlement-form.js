...
export function SellerSettlementForm({ options, initialSellerId, }) {
    const createDialog = useCreateDialog();
    const router = useRouter();
    const [isPending, startTransition] = useTransition();
    const [submitError, setSubmitError] = useState(null);
    const defaultValues = getDefaultValues(options, initialSellerId);
    const hasPayableLines = options.lines.length > 0;
    const form = useForm({
    ...
    const selectedBalance = useMemo(() => options.outstandingBalances.find((balance) => balance.sellerId === sellerId && balance.branchId === branchId) ?? options.outstandingBalances.find((balance) => balance.sellerId === sellerId), [branchId, options.outstandingBalances, sellerId]);
    const branchName = availableLines[0]?.branchName ?? selectedBalance?.branchName ?? "No branch selected";
    ...
    const remainingAfterPayment = Math.max(Number(((selectedBalance?.amountDue ?? 0) - totalAmount).toFixed(2)), 0);
    useEffect(() => {
    ...
    if (!hasPayableLines) {
        return (_jsxs("div", { className: "space-y-4", children: [_jsx("p", { className: "text-sm text-muted-foreground", children: "There are no sold received-partner items waiting for payment right now." }), _jsx("div", { className: "flex justify-end", children: _jsx(Button, { type: "button", variant: "outline", onClick: () => createDialog?.close(), children: "Close" }) })] }));
    }
    return (_jsxs("form", { className: "grid gap-3 sm:gap-6 xl:grid-cols-[2fr_1fr]", onChangeCapture: () => {
            if (submitError) {
                setSubmitError(null);
            }
        }, onSubmit: form.handleSubmit(onSubmit), children: [_jsx("input", { type: "hidden", ...form.register("branchId") }), _jsxs(Card, { children: [_jsx(CardHeader, { className: "p-4 sm:p-6", children: _jsx(CardTitle, { children: "Pay sold received items" }) }), _jsxs(CardContent, { className: "space-y-4 p-4 pt-0 sm:space-y-6 sm:p-6 sm:pt-0", children: [_jsx(FormFeedback, { errors: form.formState.errors, submitError: submitError, showValidationSummary: form.formState.submitCount > 0 }), _jsx("div", { className: "rounded-2xl border border-primary/15 bg-primary/[0.035] p-3 text-sm text-muted-foreground sm:p-4", children: "Select exact sold received-partner lines and enter the birr you are paying for each line. Assigned-from-us items are collected from the partner on the collections screen instead." }), _jsxs("div", { className: "grid gap-3 md:grid-cols-2", children: [_jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "seller-settlement-seller", children: "Partner" }), _jsx(Select, { id: "seller-settlement-seller", ...form.register("sellerId"), children: options.sellers.map((seller) => (_jsx("option", { value: seller.id, children: seller.name }, seller.id))) }), form.formState.errors.sellerId?.message ? (_jsx("p", { className: "text-xs text-destructive", children: form.formState.errors.sellerId.message })) : null] }), _jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "seller-settlement-account", children: "Payment account" }), _jsx(Select, { id: "seller-settlement-account", ...form.register("financeAccountId"), children: availableAccounts.map((account) => (_jsx("option", { value: account.id, children: formatFinanceAccountLabel(account) }, account.id))) }), form.formState.errors.financeAccountId?.message ? (_jsx("p", { className: "text-xs text-destructive", children: form.formState.errors.financeAccountId.message })) : null] })] }), _jsxs("div", { className: "grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(180px,0.7fr)]", children: [_jsxs("div", { className: "rounded-2xl bg-muted/55 p-3 sm:p-4", children: [_jsx("p", { className: "text-xs text-muted-foreground", children: "Branch" }), _jsx("p", { className: "mt-1 text-lg font-semibold", children: branchName })] }), _jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "seller-settlement-date", children: "Payment date" }), _jsx(Input, { id: "seller-settlement-date", type: "datetime-local", ...form.register("settlementDate") }), form.formState.errors.settlementDate?.message ? (_jsx("p", { className: "text-xs text-destructive", children: form.formState.errors.settlementDate.message })) : null] })] }), _jsxs("div", { className: "space-y-3 sm:space-y-4", children: [_jsx("div", { className: "flex flex-wrap items-center justify-between gap-2", children: _jsx("h3", { className: "text-sm font-semibold uppercase tracking-wide text-muted-foreground", children: "Sold lines" }) }), _jsx("div", { className: "space-y-2.5 sm:space-y-4", children: fields.map((field, index) => {
                                        ...
                                        }) }), _jsx("div", { className: "flex justify-end", children: _jsxs(Button, { type: "button", variant: "outline", size: "sm", disabled: !canAppendLine, onClick: handleAppendItem, children: [_jsx(Plus, { className: "h-4 w-4" }), "Add line"] }) })] }), _jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "seller-settlement-note", children: "Note" }), _jsx(Textarea, { id: "seller-settlement-note", rows: 3, ...form.register("note") }), form.formState.errors.note?.message ? (_jsx("p", { className: "text-xs text-destructive", children: form.formState.errors.note.message })) : null] })] })] }), _jsxs(Card, { children: [_jsx(CardHeader, { className: "p-4 sm:p-6", children: _jsx(CardTitle, { children: "Payment summary" }) }), _jsxs(CardContent, { className: "space-y-3 p-4 pt-0 sm:space-y-4 sm:p-6 sm:pt-0", children: [_jsx("div", { className: "rounded-2xl bg-muted/60 p-3 sm:p-4", children: _jsxs("div", { className: "grid grid-cols-3 gap-3", children: [_jsxs("div", { children: [_jsx("p", { className: "text-xs text-muted-foreground", children: "Lines" }), _jsx("p", { className: "mt-1 text-2xl font-semibold", children: fields.length })] }), _jsxs("div", { children: [_jsx("p", { className: "text-xs text-muted-foreground", children: "Sold qty" }), _jsx("p", { className: "mt-1 text-2xl font-semibold", children: totalQuantity })] }), _jsxs("div", { children: [_jsx("p", { className: "text-xs text-muted-foreground", children: "Sales" }), _jsx("p", { className: "mt-1 text-2xl font-semibold", children: uniqueSales.size })] })] }) }), _jsxs("div", { className: "rounded-2xl bg-muted/60 p-3 sm:p-4", children: [_jsx("p", { className: "text-xs text-muted-foreground", children: "Outstanding payable" }), _jsx("p", { className: "mt-1 text-3xl font-semibold", children: formatCurrency(selectedBalance?.amountDue ?? 0) })] }), _jsxs("div", { className: "rounded-2xl bg-muted/60 p-3 sm:p-4", children: [_jsx("p", { className: "text-xs text-muted-foreground", children: "This payment" }), _jsx("p", { className: "mt-1 text-3xl font-semibold", children: formatCurrency(totalAmount) }), _jsxs("p", { className: "mt-2 text-xs text-muted-foreground", children: ["Remaining after payment: ", formatCurrency(remainingAfterPayment)] })] }), _jsx("div", { className: "rounded-2xl bg-muted/60 p-3 text-sm text-muted-foreground sm:p-4", children: "Pay only sold received-partner lines here. Sold assigned-from-us lines should be collected from the partner on the collections screen." }), _jsxs("div", { className: "flex flex-col-reverse gap-2 sm:flex-row", children: [_jsx(Button, { type: "button", variant: "outline", className: "sm:flex-1", disabled: isPending, onClick: handleCancel, children: "Cancel" }), _jsx(Button, { className: "sm:flex-1", type: "submit", disabled: isPending, children: isPending ? "Saving..." : "Post settlement" })] })] })] })] }));
}
