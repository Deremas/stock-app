...
import { AccountType, AppRole, ExpenseStatus, LedgerDirection, LedgerEntryType, PaymentStatus, Prisma, PrismaClient, PurchaseStatus, SalePaymentMethod, SaleStatus, SettlementStatus, StockMovementType, StockOwnershipType, TransferStatus, } from "../generated/prisma/client";
...
function nextDocumentNumber(prefix, value) {
...
function lineReference(documentNumber, sku) {
    return `${documentNumber}/${sku}`;
}
function paymentStatusFromAmounts(total, amountPaid) {
...
function buildDate(base, offsetDays, hour, minute = 0) {
...
async function createCredentialUser(args) {
    const defaultBranch = must(branchesByCode.get(args.defaultBranchCode), `Missing default branch ${args.defaultBranchCode}.`);
    const branchIds = args.branchCodes.map((code) => must(branchesByCode.get(code), `Missing branch ${code}.`).id);
    ...
    await prisma.userBranch.createMany({
        data: branchIds.map((branchId) => ({
            userId: user.id,
            branchId,
            isDefault: branchId === defaultBranch.id,
            isActive: true,
        })),
    });
    ...
    usersByUsername.set(args.username, {
        id: user.id,
...
async function createOpeningBalance(args) {
    const account = must(accountsByCode.get(args.accountCode), `Missing finance account ${args.accountCode}.`);
...
async function createPurchase(seed) {
    const branch = must(branchesByCode.get(seed.branchCode), `Missing branch ${seed.branchCode}.`);
    const supplier = must(suppliersByName.get(seed.supplierName), `Missing supplier ${seed.supplierName}.`);
    const createdBy = must(usersByUsername.get(seed.createdByUsername), `Missing user ${seed.createdByUsername}.`);
    const paymentAccount = seed.paymentAccountCode
        ? must(accountsByCode.get(seed.paymentAccountCode), `Missing finance account ${seed.paymentAccountCode}.`)
        : undefined;
    const purchaseNumber = nextDocumentNumber("PUR", seed.purchasedAt);
    const subtotal = roundMoney(seed.items.reduce((sum, item) => sum + item.quantity * item.unitCost, 0));
...
async function createSupplierPayment(args) {
    const supplier = must(suppliersByName.get(args.supplierName), `Missing supplier ${args.supplierName}.`);
    const branch = must(branchesByCode.get(args.branchCode), `Missing branch ${args.branchCode}.`);
    const recordedBy = must(usersByUsername.get(args.recordedByUsername), `Missing user ${args.recordedByUsername}.`);
    const financeAccount = must(accountsByCode.get(args.financeAccountCode), `Missing finance account ${args.financeAccountCode}.`);
    const nextAmounts = paymentStatusFromAmounts(args.purchase.amountPaid + args.purchase.amountDue, args.purchase.amountPaid + args.amount);
...
async function createTransfer(args) {
    const sourceBranch = must(branchesByCode.get(args.sourceBranchCode), `Missing branch ${args.sourceBranchCode}.`);
    const destinationBranch = must(branchesByCode.get(args.destinationBranchCode), `Missing branch ${args.destinationBranchCode}.`);
    const sentBy = must(usersByUsername.get(args.sentByUsername), `Missing user ${args.sentByUsername}.`);
    const receivedBy = must(usersByUsername.get(args.receivedByUsername), `Missing user ${args.receivedByUsername}.`);
...
async function createSellerIntake(args) {
    ...
    const branch = must(branchesByCode.get(args.branchCode), `Missing branch ${args.branchCode}.`);
    const createdBy = must(usersByUsername.get(args.createdByUsername), `Missing user ${args.createdByUsername}.`);
    ...
    for (const item of args.items) {
        ...
        const intakeItem = await prisma.sellerIntakeItem.create({
            data: {
                ...
                targetSellingPrice: item.targetSellingPrice !== undefined ? money(item.targetSellingPrice) : null,
                bringingDate: args.bringingDate,
            },
            select: { id: true },
        });
        await prisma.stockMovement.create({
            data: {
                ...
                unitValue: item.targetSellingPrice !== undefined ? money(item.targetSellingPrice) : null,
...
async function createSale(args) {
    ...
    const financeAccount = args.financeAccountCode
        ? must(accountsByCode.get(args.financeAccountCode), `Missing finance account ${args.financeAccountCode}.`)
        : undefined;
    const saleNumber = nextDocumentNumber("SAL", args.soldAt);
    const subtotal = roundMoney(args.items.reduce((sum, item) => {
    ...
    const sale = await prisma.sale.create({
        data: {
            ...
            subtotal: money(subtotal),
            discountTotal: money(args.items.reduce((sum, item) => sum + item.quantity * (item.discount ?? 0), 0)),
            total: money(subtotal),
    ...
    for (const item of args.items) {
        ...
        const allocationData = {
            ...
            quantity: item.quantity,
            sourceType: item.source.kind === "purchase" || item.source.kind === "transfer"
        ...
        allocations.push({
            ...
            sellerId,
            sellerAmount: item.source.kind === "sellerIntake"
                ? item.source.sellerFixedPrice
                : item.source.kind === "sellerAssignment"
                    ? item.source.sellingPrice
                    : undefined,
            unitCost: item.source.kind === "purchase" || item.source.kind === "transfer"
                ? item.source.unitCost
                : item.source.kind === "sellerAssignment"
                    ? item.source.unitCost
                    : undefined,
            sellerAssignmentItemId: item.source.kind === "sellerAssignment" ? item.source.id : undefined,
...
async function createCustomerPayment(args) {
    const customer = must(customersByName.get(args.customerName), `Missing customer ${args.customerName}.`);
    const branch = must(branchesByCode.get(args.branchCode), `Missing branch ${args.branchCode}.`);
    const recordedBy = must(usersByUsername.get(args.recordedByUsername), `Missing user ${args.recordedByUsername}.`);
    const financeAccount = must(accountsByCode.get(args.financeAccountCode), `Missing finance account ${args.financeAccountCode}.`);
    const nextAmounts = paymentStatusFromAmounts(args.sale.amountPaid + args.sale.amountDue, args.sale.amountPaid + args.amount);
...
async function createExpense(args) {
    ...
    const resolvedCategory = must(expenseCategory, `Missing expense category ${args.expenseCategoryName}.`);
...
async function createCashTransfer(args) {
    const branch = must(branchesByCode.get(args.branchCode), `Missing branch ${args.branchCode}.`);
    const fromAccount = must(accountsByCode.get(args.fromAccountCode), `Missing finance account ${args.fromAccountCode}.`);
    const toAccount = must(accountsByCode.get(args.toAccountCode), `Missing finance account ${args.toAccountCode}.`);
...
async function createInventoryArtifacts(args) {
    ...
    for (const movement of movements) {
        ...
        ownershipEntry.stockValue = roundMoney(ownershipEntry.stockValue + movement.quantity * Number(movement.unitCost ?? 0));
    ...
    const alertRows = args.branches.flatMap((branch) => args.products.flatMap((product) => {
    ...
    const bole = must(branchesByCode.get("BOLE"), "Missing BOLE branch.");
    const boleA15Screen = must(productsBySku.get("SCR-SAM-A15-4G"), "Missing A15 screen.");
    await prisma.alertRecord.create({
        data: {
            branchId: bole.id,
            productId: boleA15Screen.id,
            alertType: "LOW_STOCK",
            threshold: boleA15Screen.minimumStockAlert,
            quantityAtAlert: 4,
            isResolved: true,
...
async function createAuditEntry(args) {
    const actor = must(usersByUsername.get(args.actorUsername), `Missing user ${args.actorUsername}.`);
...
async function main() {
    ...
    const transfer1Date = buildDate(base, -3, 9, 50);
    ...
    const cashTransferPiassaDate = buildDate(base, 0, 17, 40);
    ...
    const branchSeeds = [
        {
            code: "BOLE",
            name: "Bole Main Store",
            location: "Addis Ababa, Bole Atlas",
            phone: "+251116672211",
            email: "bole@shebamobile.et",
        },
        {
            code: "PIASSA",
            name: "Piassa Outlet",
    ...
    await createCredentialUser({
        ...
        name: "Admin",
        username: "admin",
        email: "admin@shebamobile.et",
        phone: "+251911000001",
        role: AppRole.ADMIN,
        defaultBranchCode: "BOLE",
        branchCodes: ["BOLE", "PIASSA"],
    });
    await createCredentialUser({
        password,
        name: "Sales",
        username: "sales",
        email: "sales@shebamobile.et",
        phone: "+251911000002",
        role: AppRole.SALES,
        defaultBranchCode: "BOLE",
        branchCodes: ["BOLE"],
    ...
    await createCredentialUser({
        password,
        name: "Piassa Sales",
        username: "piassa.sales",
        email: "piassa.sales@shebamobile.et",
        phone: "+251911000003",
        role: AppRole.SALES,
        defaultBranchCode: "PIASSA",
        branchCodes: ["PIASSA"],
    });
    const categoryNames = [
        "Replacement Screen",
    ...
    const financeAccountSeeds = [
        { code: "BOLE-CASH", name: "Bole Cash Drawer", type: AccountType.CASH, branchCode: "BOLE" },
        { code: "BOLE-CBE", name: "Bole CBE Collection", type: AccountType.BANK, bankName: "Commercial Bank of Ethiopia", accountNumber: "1000198765432", branchCode: "BOLE" },
        { code: "PIASSA-CASH", name: "Piassa Cash Drawer", type: AccountType.CASH, branchCode: "PIASSA" },
        { code: "PIASSA-AWASH", name: "Piassa Awash Collection", type: AccountType.BANK, bankName: "Awash Bank", accountNumber: "0112300456677", branchCode: "PIASSA" },
    ];
    for (const accountSeed of financeAccountSeeds) {
        const branch = must(branchesByCode.get(accountSeed.branchCode), `Missing branch ${accountSeed.branchCode}.`);
    ...
    await createOpeningBalance({ accountCode: "BOLE-CASH", amount: 150000, entryDate: openingBalanceDate });
    await createOpeningBalance({ accountCode: "BOLE-CBE", amount: 220000, entryDate: openingBalanceDate });
    await createOpeningBalance({ accountCode: "PIASSA-CASH", amount: 60000, entryDate: openingBalanceDate });
    await createOpeningBalance({ accountCode: "PIASSA-AWASH", amount: 90000, entryDate: openingBalanceDate });
    ...
    const purchase1 = await createPurchase({
        ...
        initialPaid: 30000,
        paymentAccountCode: "BOLE-CBE",
        note: "Samsung screens, covers, and chargers for the main branch.",
        items: [
    ...
    const purchase6 = await createPurchase({
        ...
        initialPaid: 4000,
        paymentAccountCode: "PIASSA-AWASH",
        note: "Piassa branch replenishment for walk-in accessories.",
        items: [
    ...
    const transfer1 = await createTransfer({
        sourceBranchCode: "BOLE",
        destinationBranchCode: "PIASSA",
        sentByUsername: "admin",
        receivedByUsername: "piassa.sales",
        transferAt: transfer1Date,
        note: "Fast-moving stock moved to Piassa outlet.",
        items: [
            { sku: "SCR-SAM-A24-AMOLED", quantity: 1, sourceBatch: must(purchase1.itemsBySku.get("SCR-SAM-A24-AMOLED"), "Missing Samsung A24 screen batch.") },
            { sku: "CHG-SAM-25W", quantity: 4, sourceBatch: must(purchase1.itemsBySku.get("CHG-SAM-25W"), "Missing Samsung charger batch.") },
            { sku: "COV-IPH-13-SIL", quantity: 5, sourceBatch: must(purchase2.itemsBySku.get("COV-IPH-13-SIL"), "Missing iPhone 13 cover batch.") },
            { sku: "SCR-1P-NORDCE3L", quantity: 1, sourceBatch: must(purchase5.itemsBySku.get("SCR-1P-NORDCE3L"), "Missing OnePlus screen batch.") },
        ],
    });
    ...
    const sale7 = await createSale({
        branchCode: "PIASSA",
        createdByUsername: "piassa.sales",
        soldAt: buildDate(base, -2, 16, 0),
        customerName: "Lensa Diriba",
        paymentMethod: SalePaymentMethod.CASH,
        financeAccountCode: "PIASSA-CASH",
        note: "Piassa branch walk-in accessory sale.",
        items: [
            { sku: "COV-SAM-A15-MATTE", quantity: 2, unitPrice: 290, source: must(purchase6.itemsBySku.get("COV-SAM-A15-MATTE"), "Missing Piassa A15 cover batch.") },
            { sku: "CBL-USBC-1M", quantity: 4, unitPrice: 180, source: must(purchase6.itemsBySku.get("CBL-USBC-1M"), "Missing Piassa cable batch.") },
            { sku: "CHG-APL-20W", quantity: 1, unitPrice: 1350, source: must(purchase6.itemsBySku.get("CHG-APL-20W"), "Missing Piassa Apple charger batch.") },
        ],
    });
    await createExpense({
        branchCode: "BOLE",
        ...
        note: "Monthly rent for the main branch.",
    });
    await createCashTransfer({
        branchCode: "BOLE",
        fromAccountCode: "BOLE-CBE",
        toAccountCode: "BOLE-CASH",
    ...
    const assignmentNahom = await createSellerAssignment({
        ...
        note: "Issued cables for Nahom to sell around Piassa.",
        items: [
            { sku: "CBL-USBC-1M", quantityAssigned: 12, sellingPrice: 150, sourceBatch: must(purchase6.itemsBySku.get("CBL-USBC-1M"), "Missing Piassa cable batch.") },
        ],
    });
    ...
    const sale9 = await createSale({
        ...
        paymentMethod: SalePaymentMethod.CREDIT,
        note: "Piassa branch credit sale with transferred and assigned stock.",
        items: [
            { sku: "SCR-SAM-A24-AMOLED", quantity: 1, unitPrice: 4100, source: must(transfer1.itemsBySku.get("SCR-SAM-A24-AMOLED"), "Missing transferred A24 screen batch.") },
            { sku: "COV-IPH-13-SIL", quantity: 2, unitPrice: 450, source: must(transfer1.itemsBySku.get("COV-IPH-13-SIL"), "Missing transferred iPhone 13 cover batch.") },
            ...
            { sku: "SCR-1P-NORDCE3L", quantity: 1, unitPrice: 3900, source: must(transfer1.itemsBySku.get("SCR-1P-NORDCE3L"), "Missing transferred OnePlus screen batch.") },
        ],
    });
    ...
    await createCustomerPayment({
        sale: sale9.sale,
        customerName: "Eyerusalem Kebede",
        recordedByUsername: "piassa.sales",
        branchCode: "PIASSA",
        paymentDate: customerPaymentEyerusalemDate,
        financeAccountCode: "PIASSA-AWASH",
        amount: 2000,
        note: "Initial payment against the Piassa credit sale.",
    });
    await createSupplierPayment({
    ...
    await createExpense({
        ...
        createdByUsername: "admin",
        name: "Same-day courier for Bole customer orders",
        amount: 1200,
    ...
    await createExpense({
        ...
        createdByUsername: "admin",
        name: "Piassa branch internet and power",
        amount: 2500,
    ...
    await createCashTransfer({
        branchCode: "PIASSA",
        fromAccountCode: "PIASSA-AWASH",
        toAccountCode: "PIASSA-CASH",
        transferDate: cashTransferPiassaDate,
        amount: 10000,
        note: "Pulled bank cash for Piassa walk-in operations.",
    });
    ...
    await createAuditEntry({
        ...
        entityType: "Branch",
        entityId: "BOLE,PIASSA",
        createdAt: openingBalanceDate,
        after: { branches: [...branchesByCode.keys()] },
    });
    await createAuditEntry({
        actorUsername: "admin",
    ...
    await createAuditEntry({
        ...
        entityType: "Transfer",
        entityId: transfer1.transferNumber,
        branchCode: "PIASSA",
        createdAt: transfer1Date,
        after: { sourceBranch: "BOLE", destinationBranch: "PIASSA", status: TransferStatus.RECEIVED },
    });
    await createAuditEntry({
        actorUsername: "sales",
    ...
    console.log("Admin login: admin / admin@shebamobile.et / +251911000001 / 1234");
    console.log("Bole sales login: sales / sales@shebamobile.et / +251911000002 / 1234");
    console.log("Piassa sales login: piassa.sales / piassa.sales@shebamobile.et / +251911000003 / 1234");
}
main()
    .catch((error) => {
    console.error(error);
