...
export function exportRowsToPdf(args: {
  ...
  const headerRow = args.columns
    .map(
      (column) =>
        `<th style="border:1px solid #cbd5e1;padding:10px 12px;text-align:left;background:#eff6ff;font-size:12px;">${escapeHtml(column.header)}</th>`,
    )
    .join("");
  const bodyRows =
    args.rows.length > 0
      ? args.rows
          .map((row) => {
            const cells = args.columns
              .map(
                (column) =>
                  `<td style="border:1px solid #cbd5e1;padding:10px 12px;vertical-align:top;font-size:12px;">${escapeHtml(formatTableExportValue(column, row))}</td>`,
              )
              .join("");
