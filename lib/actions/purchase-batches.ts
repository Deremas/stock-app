...
type PurchaseBatchPriceInput = z.input<typeof purchaseBatchPriceSchema>;

export async function updatePurchaseBatchSellingPriceAction(
  input: PurchaseBatchPriceInput,
  ...
  if (!parsed.success) {
    return {
      success: false,
      message:
        parsed.error.issues[0]?.message ?? "Batch selling price is invalid.",
    };
  }
  ...
  try {
    const batchReference = await prisma.$transaction(async (tx) => {
      ...
      const remainingQuantity =
        batch.quantity - soldQuantity - batch.quantityTransferred;
      ...
      const auditBranchId =
        purchaseBatch?.purchase.branchId ?? transferBatch?.transfer.destinationBranchId;
    ...
    return {
      ...
      message: getActionErrorMessage(
        error,
        "Unable to update the batch price right now.",
      ),
    };
  }
}
