    ...
    const settlementReference = await prisma.$transaction(async (tx) => {
      ...
      const normalizedItems = parsed.data.items.map((item) => {
        ...
        if (allocation.amountDue <= 0) {
          throw new Error(
            `${allocation.productName} on ${allocation.saleNumber} is already fully paid.`,
          );
        }
        ...
        if (normalizedAmount > allocation.amountDue) {
          throw new Error(
            `${allocation.productName} on ${allocation.saleNumber} has only ETB ${allocation.amountDue.toFixed(2)} left to pay.`,
          );
        }
