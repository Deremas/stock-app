...
type SellerAssignmentSourceBatch = {
  id: string;
...
async function getSellerAssignmentSourceBatch(args: {
  ...
  batchId: string;
  branchId: string;
}): Promise<SellerAssignmentSourceBatch> {
...
export async function createSellerAssignmentAction(
  ...
  if (!parsed.success) {
    return {
      success: false,
      message:
        parsed.error.issues[0]?.message ?? "Seller assignment payload did not validate.",
    };
  }
  ...
  try {
    const assignmentReference = await prisma.$transaction(async (tx) => {
      ...
      for (const item of parsed.data.items) {
        ...
        if (remainingQuantity < item.quantityAssigned) {
          throw new Error(
            `${sourceBatch.productName} has only ${remainingQuantity} item(s) left in the selected batch.`,
          );
        }
    ...
    return {
      ...
      message: getActionErrorMessage(
        error,
        "Unable to post the seller assignment right now.",
      ),
    };
  }
}
