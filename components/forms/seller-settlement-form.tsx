...
export function SellerSettlementForm({
  ...
  const selectedBalance = useMemo(
    () =>
      options.outstandingBalances.find(
        (balance) => balance.sellerId === sellerId && balance.branchId === branchId,
      ) ?? options.outstandingBalances.find((balance) => balance.sellerId === sellerId),
    [branchId, options.outstandingBalances, sellerId],
  );
  ...
  const remainingAfterPayment = Math.max(
    Number(((selectedBalance?.amountDue ?? 0) - totalAmount).toFixed(2)),
    0,
  );
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">
          There are no sold received-partner items waiting for payment right now.
        </p>
    ...
    >
      ...
      <Card>
        <CardHeader className="p-4 sm:p-6">
          <CardTitle>Pay sold received items</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 p-4 pt-0 sm:space-y-6 sm:p-6 sm:pt-0">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="seller-settlement-account">Payment account</Label>
              <Select id="seller-settlement-account" {...form.register("financeAccountId")}>
          ...
          <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(180px,0.7fr)]">
            ...
            <div className="space-y-2">
              <Label htmlFor="seller-settlement-date">Payment date</Label>
              <Input
                id="seller-settlement-date"
                type="datetime-local"
